...
static int cdrom_change_mode(int n) {
	...
	if (gdc_change_data_type(params) < 0) ret = ERR_SYS; else ret = ERR_OK;

	return ret;
}

/* Command execution sequence */
static int _cdrom_exec_cmd(int cmd, void *param) {
	int status[4];
	int f, n;
...
static int _cdrom_read_sectors(void *buffer, int sector, int cnt) {
	struct {
		int	sec, num;
		void	*buffer;
		int	dunno;
	...
	rv = _cdrom_exec_cmd(CMD_PIOREAD, &params);
...
static int cdda_read_sectors(void *buffer, int sector, int cnt)
{
	int ret;
	/* this is atomic */
	spinlock_lock(cdrom_mutex);
	if ((ret=cdrom_change_mode(CDROM_RAW))!=ERR_OK) return ret;
	if ((ret=_cdrom_read_sectors(buffer,sector,cnt))!=ERR_OK) return ret;
	if ((ret=cdrom_change_mode(CDROM_COOKED))!=ERR_OK) return ret;
	spinlock_unlock(cdrom_mutex);
	return ret;
}
