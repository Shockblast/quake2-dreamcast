...
qboolean GL_Upload32 (unsigned *data, int width, int height,  qboolean mipmap)
{
...
#ifdef	NOMEM
	scaled = alloca(scaled_width * scaled_height *sizeof(*scaled));
	if (scaled==NULL)
#else
	...
	if ( qglColorTableEXT && gl_ext_palettedtexture->value && ( samples == gl_solid_format ) )
		paletted_texture = alloca(scaled_width * scaled_height *sizeof(*paletted_texture));
#endif
